...
def read_local_table(path: Path | str) -> pl.DataFrame:
    """Read a local CSV (comma or semicolon) or Parquet file."""
    p = Path(path)
    if p.suffix.lower() == ".parquet":
        return pl.read_parquet(p)
    header = p.read_text(encoding="utf-8", errors="replace").split("\n", 1)[0]
    separator = ";" if header.count(";") > header.count(",") else ","
    return pl.read_csv(p, separator=separator)
