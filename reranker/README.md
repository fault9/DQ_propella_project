...
Evaluate trained models and baselines on a hash holdout (writes Markdown + JSON):

```powershell
learned-reranker evaluate --config configs/finepdfs_swe_latn_llm.yaml --labels traindata/LLM_train_extended.csv --external-labels traindata/human_train.csv
```

Merge LLM training shards into `LLM_train_extended.csv`:

```powershell
python scripts/merge_llm_train_extended.py
```
