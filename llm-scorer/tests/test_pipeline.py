...
# 4b. Combination: unweighted arithmetic mean of the two axes.
def test_combine_scores():
    assert abs(combine_scores(0.9, 0.9) - 0.9) < 1e-9
    assert combine_scores(0.9, 0.2) == 0.55
    assert combine_scores(0.2, 0.9) == 0.55             # symmetric
    assert combine_scores(0.0, 0.6) > 0                 # readable non-educational != garbage
