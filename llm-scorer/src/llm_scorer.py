...
from src.config import (MAX_RETRIES, BACKOFF_BASE, CHECKPOINT_EVERY, get_pricing)
from src.utils import log


def _load_env_file(path: Path) -> None:
    """Lightweight `.env` loader: KEY=value lines, ignores comments, doesn't override existing env.

    Lets background runs / freshly-launched subprocesses pick up the provider API key
    without it having to be set in the parent shell. Place the file at
    `llm-gold-standard/.env`. setdefault means a key already exported in the shell wins.
    """
    if not path.exists():
        return
...
# Auto-load `.env` from the llm-gold-standard/ project root (parent of src/).
...
def combine_scores(edu, quality):
    """Combine the two axes into the final quality_score; None if either is missing.

    Unweighted arithmetic mean of educational_value and content_quality.
    """
