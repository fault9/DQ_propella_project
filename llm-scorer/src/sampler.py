...
def sample_documents(language: str, sample_size: int, seed: int,
    ...
    for row in islice(ds, pool_size):
        if first:
            ...
            first = False
        # Keep ALL Propella columns (not just the strata fields), so the sample manifest
        # carries them through to the collaborator without a separate id-by-id rescan.
