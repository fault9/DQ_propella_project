...
# Canonical extreme-sample schema (shared by build_extreme_sample.py and run_pipeline.py).
# 18 Propella label columns + 4 FinePDFs metadata columns; the full
# CANONICAL_COLUMNS list (id, quality_score, then the two groups) mirrors
# LLM_scoring_finepdf_propella_combined.csv exactly.
PROPELLA_COLS = (
    "content_integrity", "content_ratio", "content_length",
    "one_sentence_description", "content_type", "business_sector",
    "technical_content", "information_density", "content_quality",
    "audience_level", "commercial_bias", "time_sensitivity",
    "content_safety", "educational_value", "reasoning_indicators",
    "pii_presence", "regional_relevance", "country_relevance",
)
FINEPDFS_META = (
    "full_doc_lid_score", "duplicate_count", "full_doc_lid", "minhash_cluster_size",
)
CANONICAL_COLUMNS = ["id", "quality_score"] + list(PROPELLA_COLS) + list(FINEPDFS_META)
...
DEFAULT_MODEL = "google/gemma-4-31B-it"  # default Berget judge: cheap, EU-hosted instruct model
DEFAULT_ANTHROPIC_MODEL = "claude-sonnet-4-6"        # default when provider=anthropic (--anthropic shortcut)
DEFAULT_MAX_CHARS = 3000             # chars sent to the LLM (cost lever; ~$1 for 2000 docs on Gemma-4-31B)
...
# The two LLM axes (educational_value, content_quality) are combined into the final
# quality_score as an unweighted arithmetic mean: (edu + quality) / 2. Both raw axes
# are stored, so the combine is recomputable in code without re-running the LLM.
